					...
					<div className="title-box centered">
						...
						<div className="d-flex country flex-wrap mt-3 justify-content-center">
							{[0, 1, 2, 3].map((item, index) => (
								<p className={`item ${t(`blog_photos.${item}.code`) == current ? 'current' : ''}`} onClick={() => change_current(t(`blog_photos.${item}.code`))}>{t(`blog_photos.${item}.name`)}</p>
							))}
						</div>
					</div>
				</div>
								...
								<div className="insta-carousel d-flex justify-content-center flex-wrap">
									...
									{news.map((item, index) => (
										item.country == current ?
											<div className="insta-block h-100" style={{ height: '300px' }}>
												<div className="image pt-0">
													<span className="img"><a href={item.img} className="h-100 lightbox-image" data-fancybox="insta-gallery">
														{/* <img className='h-100 px-1' src="images/uzbekistan/blog/image_1_374x292.webp" alt="" /> */}

														<div className={`dest-block-one masonry-item col-12`}>
															<div className="inner-box wow fadeInUp" data-wow-delay="0ms" data-wow-duration="1500ms">
																<div className="image-box"><a><img src={item.img} alt="" /></a></div>
